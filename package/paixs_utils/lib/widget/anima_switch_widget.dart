...
class AnimatedSwitchBuilder<T> extends StatefulWidget {
  ...
  @override
  _AnimatedSwitchBuilderState<T> createState() =>
      _AnimatedSwitchBuilderState<T>();
}

class _AnimatedSwitchBuilderState<T> extends State<AnimatedSwitchBuilder<T>>
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.value.flag) {
      case 0:
        view =
            widget.initialState ?? buildLoad(isCenter: widget.initViewIsCenter);
        break;
      case 1:
        if (widget.isCancelOtherState) {
          if (widget.defaultBuilder != null) {
            ...
            view = widget.valueAndObjBuilder!(
                widget.value.value, widget.value.object!);
          } else if (widget.listBuilder != null) {
            view = widget.listBuilder!(
          ...
          view = widget.isRef
              ? RefresherWidget(
                  ...
                  header: widget.header ??
                      buildClassicHeader(text: widget.value?.msg),
                  child: Center(
