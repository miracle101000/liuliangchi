...
class _TweenWidgetState extends State<TweenWidget> {
  ...
  Widget build(BuildContext context) {
    var view;
    if (kIsWeb || widget.isClose) return widget.child!;
    if (Platform.isAndroid || Platform.isMacOS) {
      switch (widget.axis) {
        case Axis.vertical:
          view = TweenAnimationBuilder<Offset>(
            tween: Tween<Offset>(
              begin: Offset(0, widget.value),
              end: Offset(0, 0),
            ),
              ...
              return TransformWidget.scale(
                scaleX: widget.isScale
                    ? widget.isReverseScale
                        ? (1 + (t.dy * widget.scaleX / widget.value))
                        : (1 - (t.dy / widget.value))
                    : 1,
                scaleY: widget.isScale
                    ? widget.isReverseScale
                        ? (1 +
                            (t.dy * widget.scaleY / widget.value ??
                                size(context).width))
            ...
            builder: (_, t, v) {
              return TransformWidget.scale(
                scaleX: widget.isScale
                    ? widget.isReverseScale
                        ? (1 +
                            (t.dx * widget.scaleX / widget.value ??
                                size(context).width))
                ...
                scaleY: widget.isScale
                    ? widget.isReverseScale
                        ? (1 +
                            (t.dx * widget.scaleY / widget.value ??
                                size(context).width))
                        : (1 - (t.dx / widget.value ?? size(context).width))
                    : 1,
                child: Transform.translate(offset: t, child: v),
